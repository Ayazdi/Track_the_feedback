...
def logo_analysis_on_scraped_data(df):
    """
    Analyse the posted images urls in the dataframe using the inception_v3 fine tuned
    model for logo detection and fill the logo column with the results.

    return: dataframe with logo analysis of the posts
    """
    df['logos'] = df['urls'].apply(logo_detection)
    return df
